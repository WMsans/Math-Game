if(obj_remapping_arrow.select==11){
	hover=1;
	if (obj_remapping_arrow.select==11 && keyboard_check_pressed(global.keyz)){
...
if(detecting_input_){
	text=detect_text;
	if(keyboard_key!=0){
		global.keyshift=keyboard_key;
		text=keychecks(global.keyshift);
