...
if(!setup){
	...
	if(page_num==0){
		if(cutscene_index!=[]){
			scr_create_cutscene(cutscene_index);
		}
	...
	if(draw_char==text_len[page]){
		if(page<page_num-1){
			...
			if(option_num>0 && print_op==2 && option_pos!=0){//There're options and had ended typing
				scr_create_chatbox(option_link_id[clamp(option_pos-1,0,999)],camx-64,camy);
				chosing=0;
				instance_destroy(obj_textbox_arrow);
				instance_destroy();
			}else if(option_num==0){//there's no options
				if(savebox){//This is a savebox
					instance_create_layer(-999,-999,"textbox",obj_savebox);
					instance_destroy(obj_textbox_arrow);
					instance_destroy();
...
if(!print_op)
	...
	draw_sprite(spr_chatbox_init2,txtb_img,box_x+144,box_y);
#endregion
...
if(!print_op){
	...
	if(!instance_exists(obj_textbox_arrow))
		instance_create_layer(-999,-999,"textbox_arrow",obj_textbox_arrow);
	#region draw options
	var _chosed=0;
